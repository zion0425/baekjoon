...
N과 K가 주어졌을 때, K개의 누적합을 미리 계산한다.

다음 인덱스의 K개의 누적합을 구할 때, 이전 인덱스의 K개의 누적합에 현재 인덱스 값을 더하고 현재 인덱스 - K번째 인덱스의  값을 빼준다.
...
1번째 인덱스의 누적합은 0번째 인덱스 + 1번째 인덱스의 값이다.

2번째 인덱스의 누적합은 이전에 구한 누적합에 2번째 인덱스의 값을 더하고 -K 번째 인덱스의 값을 빼주면 값이 나온다.


### 틀린 부분

test case의 값이 작을 때(0, 1) 예외 처리를 안 해줘서 문제가 발생했다.
